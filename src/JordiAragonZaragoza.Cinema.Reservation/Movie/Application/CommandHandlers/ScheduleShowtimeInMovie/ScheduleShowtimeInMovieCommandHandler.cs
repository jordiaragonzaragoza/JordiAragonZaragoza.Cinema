namespace JordiAragonZaragoza.Cinema.Reservation.Movie.Application.CommandHandlers.ScheduleShowtimeInMovie
{
    ...
    public sealed class ScheduleShowtimeInMovieCommandHandler : ICommandHandler<ScheduleShowtimeInMovieCommand>
    {
        private readonly IRepository<Movie, MovieId> movieRepository;

        public ScheduleShowtimeInMovieCommandHandler(IRepository<Movie, MovieId> movieRepository)
        {
            this.movieRepository = movieRepository ?? throw new ArgumentNullException(nameof(movieRepository));
        }

        public async Task<Result> Handle(ScheduleShowtimeInMovieCommand request, CancellationToken cancellationToken)
        {
            ...
            existingMovie.ScheduleShowtime(new ShowtimeId(request.ShowtimeId));
