namespace JordiAragonZaragoza.Cinema.Reservation.Auditorium.Application.CommandHandlers.ScheduleShowtimeInAuditorium
{
    ...
    public sealed class ScheduleShowtimeInAuditoriumCommandHandler : ICommandHandler<ScheduleShowtimeInAuditoriumCommand>
    {
        private readonly IRepository<Auditorium, AuditoriumId> auditoriumRepository;

        public ScheduleShowtimeInAuditoriumCommandHandler(IRepository<Auditorium, AuditoriumId> auditoriumRepository)
        {
            this.auditoriumRepository = auditoriumRepository ?? throw new ArgumentNullException(nameof(auditoriumRepository));
        }

        public async Task<Result> Handle(ScheduleShowtimeInAuditoriumCommand request, CancellationToken cancellationToken)
        {
            ...
            existingAuditorium.ScheduleShowtime(new ShowtimeId(request.ShowtimeId));
