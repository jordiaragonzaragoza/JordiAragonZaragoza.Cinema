namespace JordiAragonZaragoza.Cinema.Reservation.Showtime.Application.Policies.ShowtimeScheduled
{
    ...
    public sealed class ScheduleShowtimeInAuditoriumPolicy : BaseEventHandler<ShowtimeScheduledEvent>
    {
        private readonly ICommandBus commandBus;

        public ScheduleShowtimeInAuditoriumPolicy(
        ...
        public override async Task HandleAsync(ShowtimeScheduledEvent @event, CancellationToken cancellationToken)
        {
            ...
            await this.commandBus.SendAsync(new ScheduleShowtimeInAuditoriumCommand(@event.AuditoriumId, @event.AggregateId), cancellationToken);
        }
    }
}