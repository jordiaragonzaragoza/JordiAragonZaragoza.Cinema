namespace JordiAragonZaragoza.Cinema.Reservation.Showtime.Application.Policies.ShowtimeScheduled
{
    ...
    public sealed class ScheduleShowtimeInMoviePolicy : BaseEventHandler<ShowtimeScheduledEvent>
    {
        private readonly ICommandBus commandBus;

        public ScheduleShowtimeInMoviePolicy(
        ...
        public override async Task HandleAsync(ShowtimeScheduledEvent @event, CancellationToken cancellationToken)
        {
            ...
            await this.commandBus.SendAsync(new ScheduleShowtimeInMovieCommand(@event.MovieId, @event.AggregateId), cancellationToken);
        }
    }
}