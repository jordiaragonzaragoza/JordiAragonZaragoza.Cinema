namespace JordiAragonZaragoza.Cinema.Reservation.Api.Command.Controllers.V2.User
{
    ...
    public sealed class RevokeUserController : BaseApiCommandController
    {
        [HttpDelete(UserRoutes.RevokeUser)]
        [SwaggerOperation(
            Summary = "Revoke all roles from a user in a specific scope",
            Description = "Revoke all roles from a user for a specific tenant, partition, or cinema",
            OperationId = "User.RevokeUser.V2")
        ]
        public async Task<ActionResult> RevokeUserAsync(
            [FromRoute] Guid userId,
            [FromBody] RevokeUserBodyRequest request,
