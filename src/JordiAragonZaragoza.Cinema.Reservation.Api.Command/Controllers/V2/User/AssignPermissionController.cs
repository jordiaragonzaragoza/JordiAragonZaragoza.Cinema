namespace JordiAragonZaragoza.Cinema.Reservation.Api.Command.Controllers.V2.User
{
    ...
    public sealed class AssignPermissionController : BaseApiCommandController
    {
        [HttpPost(UserRoutes.AssignPermission)]
        [SwaggerOperation(
            Summary = "Assign a permission to a user in a specific scope",
            Description = "Assign a single permission to a user for a specific tenant, partition, or cinema",
            OperationId = "User.AssignPermission.V2")
        ]
        public async Task<ActionResult> AssignPermissionAsync(
            [FromRoute] Guid userId,
            [FromBody] AssignPermissionBodyRequest request,
