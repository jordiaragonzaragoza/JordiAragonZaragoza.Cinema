namespace JordiAragonZaragoza.Cinema.Reservation.Api.Command.Controllers.V2.User
{
    ...
    public sealed class RemoveRoleController : BaseApiCommandController
    {
        [HttpDelete(UserRoutes.RemoveRole)]
        [SwaggerOperation(
            Summary = "Remove a role from a user in a specific scope",
            Description = "Remove a single role from a user for a specific tenant, partition, or cinema",
            OperationId = "User.RemoveRole.V2")
        ]
        public async Task<ActionResult> RemoveRoleAsync(
            [FromRoute] Guid userId,
            [FromBody] RemoveRoleBodyRequest request,
