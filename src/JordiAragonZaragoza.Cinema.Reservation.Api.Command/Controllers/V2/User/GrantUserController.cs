namespace JordiAragonZaragoza.Cinema.Reservation.Api.Command.Controllers.V2.User
{
    ...
    public sealed class GrantUserController : BaseApiCommandController
    {
        [HttpPost(UserRoutes.GrantUser)]
        [SwaggerOperation(
            Summary = "Grant roles to a user in a specific scope",
            Description = "Grant roles to a user for a specific tenant, partition, or cinema",
            OperationId = "User.GrantUser.V2")
        ]
        public async Task<ActionResult> GrantUserAsync(
            [FromRoute] Guid userId,
            [FromBody] GrantUserBodyRequest request,
