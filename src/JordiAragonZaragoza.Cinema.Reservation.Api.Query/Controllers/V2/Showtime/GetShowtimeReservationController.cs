namespace JordiAragonZaragoza.Cinema.Reservation.Api.Query.Controllers.V2.Showtime
{
    ...
    public sealed class GetShowtimeReservationController : BaseApiQueryController
    {
        [HttpGet(ShowtimeRoutes.GetShowtimeReservation)]
        [SwaggerOperation(
            Summary = "Get an existing showtime reservation",
            Description = "Get an existing showtime reservation",
            OperationId = "Showtime.GetShowtimeReservation.V2")
        ]
        public async Task<ActionResult<ReservationResponse>> GetShowtimeReservationAsync(
            [FromRoute] GetShowtimeReservationRequest request,
